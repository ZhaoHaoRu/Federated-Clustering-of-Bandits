...
class Local_server:
    def __init__(self, nl, d, begin_num, T, server_index, edge_probability=0.8):
        ...
        self.clusters = {0: Base.CDP_Cluster(b=np.zeros(d), T=0, V=np.zeros((d,d)), users_begin = begin_num, d=d, user_num = nl, t= self.rounds, rewards= np.zeros(self.rounds), best_rewards= np.zeros(self.rounds), l_server_index=server_index, index= 0)}  # users的初始化方法直接抄了老师里面的，不知道对不对
...
class Global_server:  # 最开始每个local_server中的user数目是已知的
    def __init__(self, L, n, userList, d, T):  # 这里n指的是user的总数量，假设最开始是已知的,每个cluster的user的数量已知，存储在numlist中
        ...
        self.clusters[0] = Base.CDP_Cluster(b=np.zeros(self.d), T=0, V=np.zeros((self.d,self.d)), users_begin=0,d = self.d, user_num=self.usernum, t=self.rounds,users = {},rewards= np.zeros(self.rounds), best_rewards= np.zeros(self.rounds),l_server_index=-1,index= 0)
